...
def atten_pfa_1(x, e_lambda=1e-4, thea=1):
    input = x.size()
    #print('shape', input)
    t, b,c,  h, w = x.size()

    x = torch.reshape(x, (b * c, t, h, w)).to(x.device)
    n = w * h - 1
    x_mean = x.mean(dim=[2, 3], keepdim=True)
    x_minus_mu_square = (x - x_mean).pow(2)

    w = (thea * (x - x_mean)) / ((x - x_mean).pow(2) + 2 * e_lambda + 2 * x_minus_mu_square)
    b = (thea - w * (x + x_mean)) / 2
    y = x.mul(w) + b
    #print('x shape', y.type(), y.shape)
    y = x* nn.Sigmoid()(y)
    return torch.reshape(y, input)

class atten_pfa(nn.Module):
    def __init__(self, e_lambda=1e-4, thea=1):
        super().__init__()
    ...
    def forward(self, x):
        ...
        w = (self.thea * (x - x_mean)) / ((x - x_mean).pow(2) + 2 * self.e_lambda + 2 * x_minus_mu_square)
        b = (self.thea - w * (x + x_mean)) / 2
        y = x.mul(w) + b
        #print('x shape', y.type(), y.shape)
        y = x* nn.Sigmoid()(y)
        return torch.reshape(y, input)
