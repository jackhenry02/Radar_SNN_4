...
\begin{equation}
    \mathbf{x} = (r, \theta, \psi),
\end{equation}
where $r$ is target distance, $\theta$ is azimuth, and $\psi$ is elevation. The model observes the echo produced when an emitted acoustic signal reflects from a target. This can be viewed as a state-estimation problem: the hidden state is the target location, and the observation is a time-varying binaural waveform.
...
\begin{itemize}
    ...
    \item to evaluate the final model under clean, environmental-noise, and reverberant conditions, while identifying the limits of the constrained operating region.
\end{itemize}
...
Chapter 4 presents the results. It evaluates the fixed pathway readouts, trainable SNN readouts, input baselines, robustness to noise and reverberation, and the main error structure of the final model.
