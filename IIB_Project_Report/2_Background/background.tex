...
Auditory localization is the computational process by which an organism reconstructs the three-dimensional coordinates (azimuth, elevation, and distance) of an acoustic source from a pair of one-dimensional time-series waveforms. Unlike the visual system, where spatial coordinates map directly onto a two-dimensional retinal topology, the sensory periphery of the auditory system (the cochlea) organizes inputs purely by frequency. Consequently, spatial coordinates are completely implicit and must be actively computed by the central nervous system.
...
In passive listening, estimating the absolute distance of a source is highly error-prone because the initial source intensity is unknown. The brain must approximate range using secondary acoustic markers:
\begin{itemize}
    \item \textbf{Acoustic Attenuation:} Sound pressure level drops by roughly 6 dB for every doubling of distance following the inverse-square law.
    \item \textbf{Atmospheric Absorption:} Air acts as a natural low-pass filter over long propagation paths, preferentially absorbing high-frequency components ($f > 10\text{ kHz}$). 
    \item \textbf{Direct-to-Reverberant Ratio:} In enclosed spaces, the brain balances the energy of the direct line-of-sight sound path against the scattered, reverberant environmental reflections.
\end{itemize}

While passive systems struggle with distance estimation due to these environmental dependencies, an active echolocation system bypasses this limitation entirely. By emitting a self-generated pulse, the system establishes a precise temporal reference point ($t=0$), allowing distance to be mapped explicitly to the time-of-flight delay of the returning echo.
