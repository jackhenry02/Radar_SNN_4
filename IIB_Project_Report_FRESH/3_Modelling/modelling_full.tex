...
The input matrix was selected using Fisher information as an analytical design criterion. The attractor simulations are deterministic, but FI asks how distinguishable nearby coordinates would be if the output population were observed with small independent readout noise:
\begin{equation}
    \tilde{y}(d,t)
    =
    y(d,t)+\epsilon,
...
\begin{equation}
    ...
    \right\|_2^2 .
\end{equation}
Thus, apart from the constant scale factor $\sigma_n^{-2}$, maximising FI is equivalent to maximising the sensitivity of the output population to small changes in represented position.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{3_Modelling/CANN/input.png}
    \caption{CANN input matrices tested}
    \label{fig:CANN_input}
\end{figure}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{3_Modelling/CANN/recurrence.png}
    \caption{CANN recurrence matrix}
    \label{fig:CANN_recurrence}
\end{figure}
...
The final model used global COM because it gave the most stable sub-bin readout. In the reflected Gaussian two-block test, global COM achieved lower error than the local population-vector readout. It should be noted that whilst 0ms was the best time, this is an artifact of using a perfectly centered, clean synthetic input bump, and so using the CANN disrupts the perfect reading. The true value of the CANN emerges after 0ms when handling dynamically moving inputs or transient noise drops that occur later in the time-series.
\begin{table}[htbp]
    \centering
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.45\textwidth}
        \textbf{\large A}\\[0.5ex]  % Forces the bold 'A' to the top left
        \includegraphics[height=5cm]{3_Modelling/CANN/readout.png}
        \label{}
        \caption{}
    \end{subfigure}
    \hspace{0.1cm}
    ...
    \begin{subfigure}[t]{0.45\textwidth}
        \textbf{\large B}\\[0.5ex]  % Forces the bold 'B' to the top left
        \includegraphics[height=5cm]{3_Modelling/CANN/readout_2.png}
        \caption{}
        \label{}
    \end{subfigure}

    \caption{Readout over time comparisons of A: diagonal vs Gaussian FI input matrices, and B: local COM vs global COM}
    \label{fig:cochlea_results}
\end{figure}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{3_Modelling/CANN/alpha_gain.png}
    \caption{Alpha gain sweep with theoretical biological rate caps.}
    \label{fig:CANN_recurrence}
\end{figure}

\writingbox{
Potentially remove all FI stuff as its just not useful, relevant, or simple to explain.

Several input mappings were considered for the finite-line attractor, including
diagonal input, a two-block excitatory/inhibitory input, and a reflected
Gaussian input intended to reduce boundary artefacts. A Fisher-information
sensitivity analysis was used during development as a heuristic for comparing
these mappings. However, later ablation tests showed that the reflected input
was only marginally better than simpler diagonal alternatives, with the clearest
benefit appearing in the elevation readout. The CANN is therefore treated here
primarily as a biologically motivated population readout and stabiliser, rather
than as a major optimisation stage.
}


\section{Trainable Fusion Network}

The final implementation of the model was to use a small trainable SNN at the final readout of the model. This was to be able to implement a minimal amount of training without the need of training the full model, effectively using the feed-forward model as feature extraction. This was also compared against two baseline SNNs as a control, to see if the feature extraction based on biology improved the results relative to just training the SNN in the first case.

\subsection{Trainable Input-Only SNN Baselines}

To test whether the hand-designed auditory pathway features were genuinely useful, two input-only SNN baselines were implemented. These baselines used the same target representation, loss function, optimiser, and readout architecture as the final trainable fusion model, but removed the engineered distance, azimuth, elevation, and CANN feature extraction stages. The purpose was to determine whether a small SNN could learn the localisation task directly from earlier sensory representations.

The first baseline used the raw binaural received waveform. For each sample, the left- and right-ear waveforms were flattened and concatenated:
\begin{equation}
    x_{\mathrm{wave}}
    =
    \begin{bmatrix}
    ...
    \end{bmatrix}^{\top}.
\end{equation}
This baseline represents the least structured trainable control: the SNN receives the acoustic waveform directly, with no cochlear or pathway-level feature extraction.

The second baseline used the binaural cochlear spike raster. Each ear was passed through the same cochlear front end used elsewhere in the model, producing spike rasters $S_L(c,t)$ and $S_R(c,t)$, where $c$ indexes cochlear frequency channel and $t$ indexes time. To reduce dimensionality while preserving coarse temporal structure, each raster was divided into temporal bins and converted into spike counts:
\begin{equation}
    z_{e,c,b}
    =
    ...
    e \in \{L,R\}.
\end{equation}
The resulting left and right-ear count vectors were flattened and concatenated to form the cochlear-raster baseline input.

For both waveform and cochlear-raster inputs, two variants were tested. The direct variant fed the flattened input into the SNN without further processing. The projected variant used a fixed, non-learned Gaussian projection to compress the input to the same dimensionality as the crafted feature vector:
\begin{equation}
    z = xR,
    \qquad
    R_{ij}
    ...
    \right),
\end{equation}
where $D$ is the dimensionality of the unprojected input. This projection was included so that the baseline could be compared against the final feature-based readout without giving it a much larger first layer. Because the projection matrix was fixed and not trained, any learned mapping still had to be performed by the same small SNN readout.

The output target was identical to the final trainable model:
\begin{equation}
    y
    =
    \begin{bmatrix}
    ...
    \end{bmatrix}^{\top},
\end{equation}
where $r_{\mathrm{norm}}$ is the normalised target distance, and $\theta$ and $\psi$ are azimuth and elevation. The same uncertainty-weighted multi-task loss was used (as discussed later), so that distance, azimuth, and elevation errors were balanced automatically during training.

These baselines therefore provide a controlled comparison between four levels of representation: raw waveform input, cochlear spike input, projected versions of each, and the final hand-designed pathway feature vector. If the input-only SNNs performed similarly to the pathway-based model, this would suggest that the crafted biological feature extraction was unnecessary. If the pathway-based model performed better with the same readout architecture, this would support the claim that the neuromorphic pathway structure provides useful inductive bias and improves sample efficiency.
...
\begin{equation}
    ...
    \right].
\end{equation}
Distance is normalised to the constrained operating range, while angles are represented as sine/cosine pairs to normalise the values while also avoiding discontinuities at angular boundaries (although the tests were done in a constrained space cone). An output target of simply the normalised coordinate estimations was tested as a control, with negligible change in performance.

\subsection{Uncertainty-Weighted Loss}

The three output tasks have different units and natural scales. A manually weighted loss could be arbitrary, biased, or unbalanced, and so the readout uses learned uncertainty weighting. This eliminates the need for grid searches of weightings, keeping the scope focused on the feature extraction network rather than the trainable SNN itself. The loss is:
\begin{equation}
    \mathcal{L} =
...
\end{equation}
where $\mathcal{L}_r$ is the distance MSE, $\mathcal{L}_a$ is the azimuth sine/cosine MSE, and $\mathcal{L}_e$ is the elevation sine/cosine MSE. The learned $\sigma$ terms allow the model to balance the tasks during training. The $\log\sigma$ terms also make a lower variance of results desirable. A more complex loss function could be experimented with in future work, particularly by adding a loss related to the number of spikes to encourage sparsity and power savings. As a control, a naive implementation of an evenly weighted loss for each target error was tested with negligible changes in performance.
...
Figure \ref{fig:fusion_snn_architecture} shows the final full architecture of the trainable fusion model used with feature inputs.
